package com.zql.fileoperationlib.util;
...
public final class MediaHelper {
    ...
    private static Uri fileToContentUri(Context context, String path, String volume) {
        ...
        isMimeTypeImage = MimeTypeHelper.getMimeTypeHelperInstance().isImage(context, file);
        if (!isMimeTypeImage) {
            isMimeTypeVideo = MimeTypeHelper.getMimeTypeHelperInstance().isVideo(context, file);
            if (!isMimeTypeVideo) {
                isMimeTypeAudio = MimeTypeHelper.getMimeTypeHelperInstance().isAudio(context, file);
            }
        }
    ...
    public static String normalizeMediaPath(String path) {
        ...
        if (path.startsWith(EXTERNAL_STORAGE)) {
            final String target = new File(EMULATED_STORAGE_TARGET).getAbsolutePath();
